		...
		if (aimConf.state) {
			...
			if (aimConf.rage) {
				...
				if (aimConf.checkSpotted) {
					if (SharedFunctions::spottedCheck(C_CSPlayerPawn, localPlayer)) {
						aim::aimBot(localPlayer, baseViewAngles, baseViewAnglesAddy, CGameSceneNode.boneArray, true, client.base);
					}
				}
				else {
					aim::aimBot(localPlayer, baseViewAngles, baseViewAnglesAddy, CGameSceneNode.boneArray, true, client.base);
				...
				if (aimConf.checkSpotted) {
					if (SharedFunctions::spottedCheck(C_CSPlayerPawn, localPlayer)) {
						aim::aimBot(localPlayer, baseViewAngles, baseViewAnglesAddy, CGameSceneNode.boneArray, false, client.base);
					}
				}
				else {
					aim::aimBot(localPlayer, baseViewAngles, baseViewAnglesAddy, CGameSceneNode.boneArray, false, client.base);
